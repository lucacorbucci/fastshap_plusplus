...
def calculate_grand_coalition(dataset, imputer, batch_size, link, device, num_workers):
    ...
    """
    ones = torch.ones(
        batch_size, imputer.num_players, dtype=torch.float32, device=device
    )
    loader = DataLoader(
        dataset,
...
class FastSHAP:
    ...
    def train(
        ...
        if isinstance(train_data, np.ndarray):
            ...
            raise ValueError(
                "train_data must be np.ndarray, torch.Tensor or " "Dataset"
            )
        ...
        if optimizer_name == "adam":
            ...
            optimizer = optim.SGD(explainer.parameters(), lr=lr)
        ...
        for epoch in range(max_epochs):
            ...
            for x, grand in batch_iter:
                # Sample S.
                S = sampler.sample(
                    batch_size * num_samples, paired_sampling=paired_sampling
                )
            ...
            val_loss = (
                num_players
                * validate(
                    val_loader, imputer, explainer, null, link, normalization
                ).item()
            )
            explainer.train()
            if wandb:
