...
if __name__ == "__main__":
    ...
    X_train = dill.load(open("samples_train.pkl", "rb"))
    Y_train = dill.load(open("targets_train.pkl", "rb"))

    num_features = X_train[0].shape[0]

    X_test = dill.load(open("samples_test.pkl", "rb"))
    Y_test = dill.load(open("targets_test.pkl", "rb"))
    ...
    if args.save_model:
        ...
        torch.save(
            checkpoint,
            f"./{args.model_name}_1.pth",
        )

        torch.save(
            fastshap.explainer.state_dict(),
            f"./{args.model_name}_3.pth",
        )

        torch.save(
            fastshap.explainer,
            f"./{args.model_name}_2.pth",
        )
